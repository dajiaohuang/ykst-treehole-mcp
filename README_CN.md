...
这个命令会打开一个独立 Chrome profile。你只需要在窗口里手动完成 jAccount 登录。浏览器跳到 `https://web.treehole.space/auth/jaccount?code=...` 后，脚本会通过 Chrome 的本地调试端口读到当前 URL，把 OAuth code 换成 Treehole session，并写入 `.treehole-session.json`。
